...
\item \textbf{工具契约与确定性校验门}:9 个原生语料工具与 1 个 \texttt{delegate} 子智能体工具均通过统一 \texttt{Tool} 注册表暴露;工具声明只读性、参数校验和结果上限。调用前可拦截模型编造的论文主键(\texttt{paper\_id}),从架构上降低幻觉与捏造风险、便于审计。
...
\item \textbf{可观测运行与数据质量工程}:LangGraph 节点事件携带阶段、耗时、终止原因(\texttt{stop\_reason})和估算 token 用量;数据层通过标题优先去重、front-matter 剔除、PDF 垃圾过滤和关键词噪声过滤提升检索、趋势与图谱信噪比。
\end{enumerate}
\end{reportbox}
