...
\begin{enumerate}
\item \textbf{大模型无关的证据接地架构(LLM-agnostic, evidence-grounded)}:大语言模型(LLM)只负责"用人话写答案",领域智能沉淀在本地可复现的索引/模型文件中;生成层经 OpenAI 兼容接口\textbf{可插拔}(本地 7B ↔ DeepSeek 云端,一处配置切换),换任意大模型系统照常工作。每条回答均附引用论文,且工具调用前经\textbf{确定性校验门}拦截模型编造的论文主键(\texttt{paper\_id}),从架构上降低幻觉与捏造风险、便于审计。
