...
\subsection{运行环境}
\begin{itemize}
\item Python 3.11(conda 环境),Node.js(前端);PostgreSQL 15 + pgvector 扩展。
...
\subsection{质量与效果验证}
\begin{plainbox}\ttfamily\small
make test\hfill\# 92 项后端测试 + 前端类型检查 + 构建\\
...
\subsection{交付物清单}
\begin{itemize}
\item \textbf{代码}:\texttt{src/}(采集/治理/分析/模型)、\texttt{backend/}(API)、\texttt{frontend/}(工作台)、\texttt{infra/}(schema)、\texttt{Makefile}(流水线)。
\item \textbf{模型文件}:pgvector 向量索引、\texttt{models/}(嵌入器/趋势/推荐)、\texttt{graphs/}(图谱 JSON)、RAG 片段语料;均可由 \texttt{make agent-build} 复现。
