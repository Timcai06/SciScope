...
\begin{plainbox}
\textbf{下一阶段产品化路线}:当前智能体编排为轻量 ReAct 式运行时,已覆盖 plan、只读工具并行、去重、reflect、retry 与 SSE 事件化输出。后续产品化将把编排层迁移为 LangGraph StateGraph,以获得标准化状态持久化、Human-in-the-Loop、节点级恢复与多 Agent 分工;现有检索、趋势、推荐、图谱和 \texttt{verify\_claim} 工具保持不变,迁移目标是升级运行时而非重写数据底座。
\end{plainbox}
...
.3 \texttt{agent/}\quad [4] ReAct 工具循环(plan->执行->观察->反思,9 工具).
...
4. 智能体 & ReAct 工具循环:plan -> 选工具 -> 并行执行 -> 观察 -> 反思自评 -> 据证作答(\texttt{backend/app/agent}) \\
