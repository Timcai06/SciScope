...
\begin{plainbox}
\textbf{当前产品化路线}:智能体编排已以 LangGraph StateGraph 作为默认运行时,将 prepare、plan、LLM step、tool execution、reflect 与 final synthesis 拆为可观察节点;legacy ReAct loop 仅保留为兼容回退。下一阶段将在该图上补充 checkpoint/session state、Human-in-the-Loop、节点级恢复与多 Agent 分工;现有检索、趋势、推荐、图谱和 \texttt{verify\_claim} 工具保持不变,迁移目标是升级运行时而非重写数据底座。
\end{plainbox}
...
\item \textbf{智能体编排}:基于 LangGraph StateGraph 的科研智能体运行时——prepare / plan / LLM step / tool execution / reflect / final synthesis 节点化编排,底层使用本地 \texttt{Qwen2.5-7B}(vLLM,hermes 工具解析),支持流式选工具、只读工具并行执行、调用去重、plan/reflect 自校验,事件化输出供 CLI 与 Web 复用。
\end{itemize}
...
.3 \texttt{agent/}\quad [4] LangGraph StateGraph(plan->执行->观察->反思,9 工具).
.3 \texttt{api/}\quad [5] \texttt{/api/agent/stream}(SSE)+ search / chat / trends.
...
4. 智能体 & LangGraph StateGraph:prepare -> plan -> 选工具 -> 并行执行 -> 观察 -> 反思自评 -> 据证作答(\texttt{backend/app/agent}) \\
5. 接口 & FastAPI;智能体经 \texttt{/api/agent/stream} 以 SSE 流式输出(\texttt{backend/app/api}) \\
