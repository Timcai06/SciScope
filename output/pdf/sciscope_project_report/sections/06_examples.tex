...
下列样例来自\textbf{固定验收会话、历史运行记录或当前工程基线},用于直观展示检索、跨语言、推荐与证据接地能力。它们证明"系统如何工作",不自动构成更高层的质量主表、人工 Gold 或真实用户成效证明。
...
\subsection{用例四:趋势热点(描述性输出)}
\begin{plainbox}
\textbf{描述性热点排行}(关键词清洗后):
\texttt{retrieval augmented generation} → \texttt{large language model} → \texttt{gene delivery} → \dots\par\vspace{2pt}
与 2022--2026 年科研态势相符; 但根据当前滚动回测,这类输出只可用于\textbf{描述历史方向和候选热点},不得写成对下一年度的确定预测。
\end{plainbox}
...
下面是终端智能体处理一个复合问题的\textbf{真实执行轨迹}(本地 Qwen2.5-7B;演示服务器当前为 DeepSeek,编排行为一致),展示"规划 → 自主取证 → 多步链式 → 据证作答":
\begin{plainbox}
\textbf{提问}:推荐几篇关于知识图谱的代表论文并说明趋势\par\vspace{3pt}
\statusbadge{1. 规划} 自动生成计划:先检索代表论文 → 再基于真实论文做推荐 → 最后查询"knowledge graph"趋势\par\vspace{2pt}
\statusbadge{2. 执行} 智能体依计划链式取证:
\begin{enumerate}[itemsep=1pt]
\item 跨语言检索"知识图谱",命中英文文献并取得真实论文编号;
\item 以检索得到的真实论文为种子做推荐,\textbf{不是预设固定结果};
\item 将中文关键词映射到英文研究术语,查询年度趋势。
\end{enumerate}
\statusbadge{3. 据证作答} 综合三个工具的真实结果,给出代表论文清单 + 趋势判定,并标注出处。\par\vspace{2pt}
\textbf{关键特性}:计划对用户可见;推荐使用的是\textbf{检索得到的真实论文编号而非臆造};相同参数调用自动去重防空转;回答后还会\textbf{自评证据是否充分},不足则改写重试。
\end{plainbox}

\subsection{用例七:论断核查与跨语言接地}
...
\textbf{工具输出}(历史接地样例):支持等级=\textbf{强支持},最高接地相似度=\textbf{0.846}\par\vspace{2pt}
\textbf{主要证据}:
\begin{enumerate}[itemsep=1pt]
...
\end{enumerate}
\textbf{关键特性}:中文论断可由英文论文接地;工具返回的是\textbf{支持 / 反驳 / 证据不足}及其校准边界,要求模型据证表述,证据不足时不得强行断言。该样例用于展示证据链,不等于正式人工 Gold 质量证明。
\end{plainbox}
...
\end{tabularx}
\end{tableblock}
\textbf{输出边界}:结论表述为"降低幻觉风险并提升可核验性";证据工具与时间线入口保留给评委复核。
\end{plainbox}
