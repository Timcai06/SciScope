...
- 状态：`active`（D00 已完成盘点，验收待项目负责人复核；其余任务 `PENDING`）
- 负责人：数据/后端负责人；复核：领域专家 + 项目负责人
- 当前领取：D00（复核）→ D01；上游：[冻结目标说明书](../../project/国赛目标说明书.md)
...
> [D-数据准入清单](D-数据准入清单.md)（D00 产物，盘点基线 `c63ef49`）。
...
| canonical 数据无内容哈希，仅规范化时间戳 | `src/harvest/raw_governance.py`（rmtree 式全量重建） | “任一论文可回指来源**和版本**”中的“版本”已成立（须 D01/D02 补校验和） |
...
- 验收：任一论文可从运行时记录回指来源和版本；未知许可一律不能进入公开导出。
- 验证：`rtk git diff --check`；不因盘点而修改语料。
...
- 验证：使用任务实现后新增的最小导入测试；命令和退出码写回本节。
...
| 任务 | 状态 | PASS 所需证据 |
|---|---|---|
| D00 | `PENDING` | 来源/许可/版本台账，经项目负责人复核 |
| D01 | `PENDING` | 合同测试与拒绝日志 |
