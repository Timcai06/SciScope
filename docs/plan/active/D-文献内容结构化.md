...
- 状态：`active`（D00 `PASS`；D01 `DONE` 待复核；其余任务 `PENDING`）
- 负责人：数据/后端负责人；复核：领域专家 + 项目负责人
- 当前领取：D01（复核）→ D02；上游：[冻结目标说明书](../../project/国赛目标说明书.md)
...
- 状态：`DONE`（产物已生成；2026-08 已按评审修订双哈希语义与类型校验，验收待项目负责人复核）；依赖：D00。
...
| D01 | `DONE`（待复核） | [交付记录](./D/D01-受控摄取数据合同.md)：合同测试 15 passed + 拒绝日志；实现 `src/data_contracts/admission.py`、字典 `src/data_contracts/ADMISSION.md` |
