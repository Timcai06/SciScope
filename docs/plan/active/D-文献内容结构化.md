...
- 状态：`active`（D00–D03 `PASS`；D04、D05 `DONE` 待复核；D 线实现全部完成）
...
- 状态：`DONE`（产物已生成；**领域专家盲审未执行**，验收待项目负责人复核）；依赖：D03。
...
- 验证：`rtk test python3 -m pytest backend/tests/test_extraction_eval.py` → **5 passed**；
  `rtk test make test-backend` → 全量绿。实现：`src/infra/extraction_eval.py`
  （每字段 n/存在率/正确率/可定位率/拒答率/留空率/Wilson CI/可复查失败例）；
  报告：[D04-20260805-抽取质量与数据卡](D/D04-20260805-抽取质量与数据卡.md)（含 Data Card 与盲审模板）。
  **盲审 + gold 标注未执行（人工项），字段级正确率待领域专家回填。**
...
| D04 | `DONE`（待复核；盲审待领域专家） | [Data Card 与评测表](./D/D04-20260805-抽取质量与数据卡.md)：聚合脚本 + 5 项测试 + Wilson CI + 可复查失败例；gold/盲审待人工回填 |
